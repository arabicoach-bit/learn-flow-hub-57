...
import { usePackageComments, useAddPackageComment } from '@/hooks/use-package-comments';
...
export function PackageCommentsDialog({ open, onOpenChange, packageId, packageLabel }: PackageCommentsDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addComment.mutateAsync({ packageId, comment: newComment.trim() });
      setNewComment('');
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-base">Package Notes — {packageLabel}</DialogTitle>
        </DialogHeader>
