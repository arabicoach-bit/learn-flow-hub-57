...
import {
  CalendarDays, Users, Clock, Check, X, Sun, Sunrise,
  AlertCircle, Search, ListFilter, CheckCircle, XCircle,
  Loader2, ArrowUpDown, MessageSquare,
} from 'lucide-react';
...
function useTrialLessonFullData(teacherId: string, isAdmin: boolean, startDate: string | null, endDate: string | null) {
  return useQuery({
    queryKey: [isAdmin ? 'admin-teacher-trial-full' : 'teacher-trial-full', teacherId, startDate, endDate],
    queryFn: async () => {
      let query = supabase
        .from('trial_lessons_log')
        .select(`
          trial_lesson_id, trial_student_id, lesson_date, lesson_time,
          duration_minutes, status, notes, teacher_payment_amount,
...
function getConversionBadge(status?: string) {
  switch (status) {
    case 'Converted': return <Badge className="bg-emerald-500/20 text-emerald-600 border-emerald-500/30 text-xs">Converted</Badge>;
    case 'Lost': return <Badge className="bg-destructive/20 text-destructive border-destructive/30 text-xs">Lost</Badge>;
    default: return <Badge className="bg-amber-500/20 text-amber-600 border-amber-500/30 text-xs">Pending</Badge>;
  }
}
...
export function TrialLessonCalendar({ teacherId, isAdmin }: TrialLessonCalendarProps) {
  ...
  const { data: allLessons = [], isLoading, refetch } = useTrialLessonFullData(teacherId, !!isAdmin, startDate, endDate);
  ...
  const stats = useMemo(() => {
    ...
    // Conversion rate: converted students / completed students (unique)
    ...
    return { total, scheduled, completed, absent, todayCount, unmarked, pending, converted, lost, conversionRate, attendanceRate };
  }, [allLessons, todayStr]);
          ...
          <CardContent>
            <div className="flex items-center gap-6">
              ...
              <div className="grid grid-cols-3 gap-x-6 gap-y-2 flex-1">
                <div><p className="text-2xl font-bold">{isLoading ? '-' : stats.total}</p><p className="text-xs text-muted-foreground">Total</p></div>
                <div><p className="text-2xl font-bold text-emerald-500">{isLoading ? '-' : stats.completed}</p><p className="text-xs text-muted-foreground">Completed</p></div>
                <div><p className="text-2xl font-bold text-destructive">{isLoading ? '-' : stats.absent}</p><p className="text-xs text-muted-foreground">Absent</p></div>
                <div><p className="text-2xl font-bold text-purple-500">{isLoading ? '-' : stats.scheduled}</p><p className="text-xs text-muted-foreground">Scheduled</p></div>
                <div><p className="text-2xl font-bold text-amber-500">{isLoading ? '-' : stats.todayCount}</p><p className="text-xs text-muted-foreground">Today</p></div>
                <div><p className={`text-2xl font-bold ${stats.unmarked > 0 ? 'text-destructive' : ''}`}>{isLoading ? '-' : stats.unmarked}</p><p className="text-xs text-muted-foreground">Unmarked</p></div>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card className="border-emerald-500/20">
          ...
          <CardContent>
            <div className="flex items-center gap-6">
              ...
              <div className="grid grid-cols-3 gap-x-6 gap-y-2 flex-1">
                <div><p className="text-2xl font-bold text-amber-500">{isLoading ? '-' : stats.pending}</p><p className="text-xs text-muted-foreground">Pending</p></div>
                <div><p className="text-2xl font-bold text-emerald-500">{isLoading ? '-' : stats.converted}</p><p className="text-xs text-muted-foreground">Converted</p></div>
                <div><p className="text-2xl font-bold text-destructive">{isLoading ? '-' : stats.lost}</p><p className="text-xs text-muted-foreground">Lost</p></div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="glass-card">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg flex items-center gap-2">
                ...
                {todayLessons.length > 0 && <Badge variant="outline" className="ml-2">{todayLessons.length} trial{todayLessons.length !== 1 ? 's' : ''}</Badge>}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
        ...
        <TabsContent value="list" className="space-y-4 mt-4">
          <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="w-[150px]"><SelectValue placeholder="Attendance" /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={conversionFilter} onValueChange={setConversionFilter}>
              <SelectTrigger className="w-[150px]"><SelectValue placeholder="Conversion" /></SelectTrigger>
              <SelectContent>
                              ...
                              {lesson.status === 'scheduled' ? (
                                <div className="flex items-center gap-1 justify-center">
                                  <Button
                                    ...
                                    disabled={isThisUpdating} title="Mark Completed"
                                  >
                                  ...
                                  <Button
                                    ...
                                    disabled={isThisUpdating} title="Mark Absent"
                                  >
          ...
          <Card className="glass-card">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2">
                <CalendarDays className="w-5 h-5 text-purple-500" /> Trial Lessons Calendar
              </CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
          ...
          <Card className="glass-card">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {selectedDate && isTodayFn(selectedDate) && <Badge className="bg-purple-500/20 text-purple-500 border-purple-500/30">Today</Badge>}
