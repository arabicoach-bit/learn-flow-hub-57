...
export function useAddPackageWithSchedule() {
  ...
  return useMutation({
    mutationFn: async (input: AddPackageWithScheduleInput) => {
      ...
      // 2. Calculate next payment date (30 days from start)
      ...
      // 4. Insert weekly schedule template
      ...
      // 5. Generate all scheduled lessons using the database function
      ...
      // 6. Recalculate wallet from DB (single source of truth)
      await supabase.rpc('recalculate_student_wallet', { p_student_id: input.student_id });

      // 7. Update student metadata (non-wallet fields)
