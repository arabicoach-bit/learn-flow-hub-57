...
BEGIN
  ...
  IF p_status IN ('Taken', 'Absent') THEN
    -- Both Taken and Absent deduct 1 from wallet
    v_new_wallet := v_wallet - 1;
    
    IF p_status = 'Taken' AND v_wallet >= 1 THEN
    ...
    ELSIF p_status = 'Absent' THEN
      v_package_to_use := v_current_package;
      -- For absent, increment lessons_used but don't count toward salary
      UPDATE packages SET lessons_used = lessons_used + 1 
      WHERE package_id = v_current_package;
    ELSE
    ...
    -- Status thresholds: Active (>=3), Grace (2 to -1), Blocked (<=-2)
