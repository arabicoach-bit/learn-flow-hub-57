...
-- Fix the generate_package_schedule function to check for existing lessons before inserting
...
BEGIN
      ...
      IF v_lessons_scheduled < p_total_lessons THEN
        ...
        IF NOT EXISTS (
          ...
          WHERE package_id = p_package_id
            ...
            AND status IN ('scheduled', 'completed', 'rescheduled')
        ) THEN
  ...
  RETURN json_build_object(
    ...
    'new_lessons_added', v_lessons_scheduled - v_existing_count,
